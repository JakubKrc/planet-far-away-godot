...
func move(params : Dictionary):
	...
	if params['x_axis_input']==1:
		...
		if !parent.get_component(State).state == Global.States.FLYING:
			parent.animation_player.play("walk_right")
			parent.get_component(State).state = Global.States.MOVING_RIGHT
	if params['x_axis_input']==-1:
		parent.direction = Vector2.LEFT
		if !parent.get_component(State).state == Global.States.FLYING:
			parent.animation_player.play("walk_left")
			parent.get_component(State).state = Global.States.MOVING_LEFT
